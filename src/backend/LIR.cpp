...
void LIR::run(){
    ...
    breakGEP(getBasicBlocks(moudle_));
    oppositeJ(getBasicBlocks(moudle_));
    makeOffset(getBasicBlocks(moudle_));
...
void LIR::makeOffsetT(T*inst_load_or_store, ::std::list<Instruction*>& inst_list, ::std::list<Instruction*>::iterator& inst_pos, BasicBlock*bb){
    if constexpr (std::is_same_v<T, LoadInst>){
        ...
        else if(dynamic_cast<Instruction*>(lval))   handleAdd(dynamic_cast<Instruction*>(lval), inst_list, inst_pos, bb, 0);
    }
    else{
        auto lval = inst_load_or_store->getLVal();
        if(dynamic_cast<GetElementPtrInst*>(lval))  handleGEP(dynamic_cast<GetElementPtrInst*>(lval), inst_list, inst_pos, bb, 1);
        else if(dynamic_cast<Instruction*>(lval))   handleAdd(dynamic_cast<Instruction*>(lval), inst_list, inst_pos, bb, 1);
    }
}
...
void LIR::handleAdd(Instruction* inst_ptr, ::std::list<Instruction*>& inst_list, ::std::list<Instruction*>::iterator& inst_pos, BasicBlock* bb, bool flag){
    Value *base, *offset;
    auto inst = *inst_pos;
    if(inst_ptr->isAdd()){
